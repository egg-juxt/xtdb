(ns scenarios.load-test
  (:require [clojure.java.io :as io]
            ...
            [scenarios.commons :refer [*node* batch-submits execute-tx! for-valid-time interleave-submits-2 make-node-conf node-small periodic-instants reset-small! set-node!]]
            [xtdb.api :as xt]
            [xtdb.node :as xtnode]
            [xtdb.util :as xtutil])
...
(defn seed-readings [{:keys [system-count from to]}]
  (let [dates (periodic-instants (t/duration 5 :minutes) from to)]
    (->> (let [gen-value (constantly 1000) #_#(rand-int-between 1000 5000)]
           (for [[from to value] (map vector
                                   dates (next dates) (repeatedly 1000 gen-value))
...
(comment
  (seed-readings {:system-count 2
                  :from (t/instant "2024-01-01T00:00:00Z")
                  :to (t/instant "2024-01-01T00:30:00Z")}))
...
(defn add-days-off-ops [put-docs off-data]
  (let [table (-> put-docs second :into)
        data (nth put-docs 2)
        v-from (-> put-docs second :valid-from)
        v-to (-> put-docs second :valid-to)]
    (concat
      [put-docs]
...
(defn seed-system-ops [id vfrom vto]
  (concat
    (add-days-off-ops [:put-docs {:into :site, :valid-from vfrom, :valid-to vto}
                       {:xt/id (str "site" id)
                        :prop1 "yes"}]
                      {:prop1 "no"})
    (add-days-off-ops [:put-docs {:into :system, :valid-from vfrom, :valid-to vto}
                       {:xt/id id
                        :nmi (str "site" id)
                        :prop1 "yes"}]
                      {:prop1 "no"})
    (add-days-off-ops [:put-docs {:into :device, :valid-from vfrom, :valid-to vto}
...
(defn seed-systems [{:keys [system-count from to]}]
  (->> (for [system-id (range system-count)]
         (seed-system-ops system-id (t/instant from (t/zone-id "UTC")) (t/instant to (t/zone-id "UTC"))))
    (apply concat)
    (map (fn [put-docs]
...
(comment
  (seed-systems {:system-count 2
                 :from (t/instant "2024-01-01T00:00:00Z")
                 :to (t/instant "2024-01-08T00:00:00Z")}))

(defn seed-scenario [opts]
  (doseq [submit-args (batch-submits 1000
                        (interleave-submits-2
                          (seed-systems opts)
                          (seed-readings opts)))]
    (apply execute-tx! submit-args)))

(comment
  (reset-small!)
  (seed-scenario {:system-count 2
  ...
  (set-node! node-load1)
  (seed-scenario {:system-count 500
                  :from (t/local-date-time 2024 1 1 0 0)
...
; Filtering issue
; https://github.com/xtdb/xtdb/issues/4097
...
(comment
  ...
  (seed-systems-only {:system-count 2000
                      :from (t/local-date-time 2024 1 1 0 0)
  ...
  (->> (xt/q *node*
    ...
    (map (juxt :xt/id :xt/valid-time :prop1)))
    ...
    "SELECT site_id, system_id, overlap
       FROM (
         SELECT
           ...
           system._valid_time * site._valid_time * device._valid_time
             AS overlap,
         FROM site FOR VALID_TIME FROM DATE '2024-01-01' TO DATE '2024-01-30'
         JOIN system FOR VALID_TIME FROM DATE '2024-01-01' TO DATE '2024-01-30'
           ON system.nmi = site._id
         JOIN device FOR VALID_TIME FROM DATE '2024-01-01' TO DATE '2024-01-30'
...
(defn vpp-metric--query [{:keys [] :as opts}]
  (str
    "WITH vpp_system_range AS (" (vpp-systems-range--query opts) ")
     FROM (
       FROM
         reading FOR VALID_TIME FROM DATE '2024-01-01' TO DATE '2024-01-30'
         JOIN vpp_system_range ON vpp_system_range.system_id = reading._id
       WHERE
...
(comment
  (xt/q *node* (vpp-systems-range--query {:system-count 2}) #_{:explain? true})

  (->> (xt/q *node* (vpp-systems-range--query {:system-count 1}) {:explain? true})
    (sort-by :system-id)
...
(comment
  ...
  (time (seed-readings-ops node-load1))
